...
class AppTestCase(unittest.TestCase):
    ...
    def test_duplicate_reviews(self):
        response1 = self.app.post('/api/v2/auth/2/reviews', data = json.dumps(self.data2) , content_type = 'application/json')
        result1 = json.loads(response1.data.decode())
        self.assertEqual(result1["message"], "Review added Successfully")
        self.assertEqual(response1.status_code, 201)
        response2 = self.app.post('/api/v2/auth/2/reviews', data = json.dumps(self.data2) , content_type = 'application/json')
