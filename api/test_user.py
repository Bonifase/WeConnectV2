...
class AppTestCase(unittest.TestCase):
    ...
    def test_user_login(self):
        response = self.app.post('/api/v1/auth/user_login', data = json.dumps(self.data2) , content_type = 'application/json')
        result = json.loads(response.data.decode())
        self.assertEqual(result["message"], "Login Successful")
        self.assertEqual(response.status_code, 200)
    def test_unregistered_user(self):
        response = self.app.post('/api/v1/auth/user_login', data = json.dumps(self.data3) , content_type = 'application/json')
        result = json.loads(response.data.decode())
    ...
    def test_reset_password(self):
        ...
        self.assertEqual(response1.status_code, 201)
        response2 = self.app.post('/api/v1/auth/user_login', data = json.dumps(self.data6) , content_type = 'application/json')
        result2 = json.loads(response2.data.decode())
    ...
    def test_logout(self):
        ...
        result = json.loads(response.data.decode())
        self.assertEqual(response.status_code, 200)
        response1 = self.app.post('/api/v1/auth/user_logout', data = json.dumps(self.data2) , content_type = 'application/json')
        result1 = json.loads(response1.data.decode())
