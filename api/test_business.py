...
class AppTestCase(unittest.TestCase):
    ...
    def test_create_business(self):
        response = self.app.post('/api/v2/auth/businesses', data = json.dumps(self.data), content_type = 'application/json')
        result = json.loads(response.data.decode())
        self.assertIn(self.data['name'], result['name'])
        self.assertEqual(response.status_code, 201)

    def test_duplicate_business(self):
        response1 = self.app.post('/api/v2/auth/businesses', data = json.dumps(self.data2) , content_type = 'application/json')
        self.assertEqual(response1.status_code, 201)
        response2 = self.app.post('/api/v2/auth/businesses', data = json.dumps(self.data2) , content_type = 'application/json')
    ...
    def test_view_businesses(self):
        response = self.app.get('/api/v2/auth/businesses', data = json.dumps(self.data), content_type = 'application/json')
        result = json.loads(response.data.decode())
