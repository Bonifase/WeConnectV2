...
class AppTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with app.app_context():
            ...
            self.login_user = self.app.post(
                '/api/v2/auth/login', data=json.dumps(self.data3), content_type='application/json')
            result = json.loads(self.login_user.data.decode())
            self.user_token = result['access_token']
            self.app.post('/api/v2/businesses', headers={"Authorization": 'Bearer ' +
                                                         self.user_token}, data=json.dumps(self.data4), 
                                                         content_type='application/json')

    def test_add_reviews_works(self):

        response = self.app.post(
            'api/v2/1/reviews', data=json.dumps(self.data), headers={"Authorization": 'Bearer ' + self.user_token},
             content_type='application/json')
    ...
    def test_add_reviews_for_unavailable_business_fails(self):
        ...
        response = self.app.post(
            '/api/v2/5/reviews', data=json.dumps(self.data), 
    ...
    def test_available_reviews_works(self):
        ...
        self.app.post('/api/v2/1/reviews',data=json.dumps(self.data), 
          headers={"Authorization": 'Bearer ' + self.user_token}, content_type='application/json')
        response = self.app.get('/api/v2/1/reviews', data=json.dumps(self.data), 
          headers={"Authorization": 'Bearer ' + self.user_token}, content_type='application/json')
        result = json.loads(response.data.decode())
    ...
    def test_unvailable_reviews_fails(self):
        ...
        self.app.post('/api/v2/1/reviews',
                      data=json.dumps(self.data), headers={"Authorization": 'Bearer ' + self.user_token},
                       content_type='application/json')
