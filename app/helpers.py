...
def confirm_reset_password():
    confirm_reset_password = app.route(
        '/api/v2/auth/reset-password/<string:reset_token>', methods=['POST'])
...
def retrieve_with_pagination():
    pagination = app.route(
        '/api/v2/businesses/<int:page_num>/', methods=['GET'])
    return pagination
...
def update_business():
    update_business = app.route(
        '/api/v2/businesses/<int:businessid>', methods=['PUT'])
    return update_business
""""DELETE BUSINESS ROUTE"""


def delete_business():
    delete_business = app.route(
        '/api/v2/businesses/<int:id>', methods=['DELETE'])
...
def search_business():	
    search = app.route(
        '/api/v2/businesses/search', methods=['GET'])
    return search
""""ADD BUSINESS REVIEW ROUTE"""


def add_business_review():
    add_review = app.route(
        '/api/v2/<int:businessid>/reviews', methods=['POST'])
    return add_review
""""RETRIEVE BUSINESS REVIEWS ROUTE"""


def retrieve_all_business_reviews():
    get_reviews = app.route(
        '/api/v2/<int:businessid>/reviews', methods=['GET'])
    return get_reviews
